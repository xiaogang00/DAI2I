...
class Dataset_CRGAN(base_dataset.BaseDataset):

    def __init__(self, image_list, transform=forward_transform, scale=(128, 128), crop_size=(0, 0),
                 bias=(0, 0), aug=False, return_filename=False, aug_param={}):
        super(Dataset_CRGAN, self).__init__()
        self.files = []
        supported_format = ['jpg', 'png', 'jpeg']
        for image_now in image_list:  # filter out files that are not image
            format = image_now.split('.')[-1]
        ...
        if 'resize_param' not in aug_param:
            aug_param['resize_param'] = None
        if 'rotation_param' not in aug_param:
            aug_param['rotation_param'] = None
        if 'shift_param' not in aug_param:
            aug_param['shift_param'] = None
        if 'crop_param' not in aug_param:
            aug_param['crop_param'] = None
        if 'color_param' not in aug_param:
            aug_param['color_param'] = None
    ...
    def __getitem__(self, index):
        img = util.readRGB(self.files[index]).astype(np.float32)
        img = np.pad(img, ((0, 70), (0, 0), (0, 0)), 'constant', constant_values=255)
...
class Dataset_PIE2(base_dataset.BaseDataset):
    def __init__(self, image_list, transform=forward_transform, scale=(128, 128), crop_size=(0, 0),
                 bias=(0, 0), aug=0, return_filename=False, aug_param={}):
        super(Dataset_PIE2, self).__init__()
        self.files = []
        supported_format = ['jpg', 'png', 'jpeg']
        for image_now in image_list:  # filter out files that are not image
            format = image_now.split('.')[-1]
    ...
    def __getitem__(self, index):
        ...
        views = ['_190_', '_041_', '_050_', '_051_', '_140_', '_130_', '_080_']
        base_name = os.path.basename(self.files[index])
        attribute = [0] * len(views)
        for mm in range(len(views)):
            if views[mm] in base_name:
                attribute[mm] = 1
