...
class Engine(object):
    ...
    def define_model(self):
        ...
        ## CR-GAN
        base_model = Adapt_Net.view_point_transfer()
        G_path = os.path.join('I2I_model/netG_xvz_aug.pth')
        print(G_path)
        base_model.G_xvz.load_state_dict(torch.load(G_path))
        G_path = os.path.join('I2I_model/netG_vzx_aug.pth')
        print(G_path)
        base_model.G_vzx.load_state_dict(torch.load(G_path))

        ## generator
        from network import adain_net
        from torch import nn
        G = adain_net.Generator_split(**self.config['generator'])
        G = nn.DataParallel(G)
    ...
    def load_dataset(self):
        ...
        import glob
        dst_list = glob.glob(self.config['dst_dir1'] + '/*.jpg')
        dst_list += glob.glob(self.config['dst_dir2'] + '/*.jpg')

        train_dataset = datasetSimple.Dataset_PIE2(image_list=train_list,
                                                   crop_size=(self.config['src_crop_size'], self.config['src_crop_size']),
                                                   aug=self.config['src_aug'], aug_param=self.config['aug_param'])
        dst_dataset = datasetSimple.Dataset_CRGAN(image_list=dst_list,
